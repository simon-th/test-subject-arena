...
void ST7735_DrawBitmap(int16_t x, int16_t y, const uint8_t *image, int16_t w, int16_t h){
  ...
  for(y=0; y<h; y=y+1){
    for(x=0; x<w; x=x+1){
      uint16_t pix = image[i];
			pix = ((pix << 8) & 0xE000) + ((pix << 6) & 0x0700) + ((pix << 3) & 0x0018);
...
void ST7735_OutChar(char ch){
  ...
  ST7735_DrawCharS(StX*6,StY*10,ch,ST7735_CYAN,ST7735_BLACK, 1);
  StX++;
  if(StX>20){
    StX = 20;
    ST7735_DrawCharS(StX*6,StY*10,'*',ST7735_RED,ST7735_BLACK, 1);
...
// Function to replace background pixel based on the index in sprite array
...
void bitmapBuffer(int16_t x, int16_t y, uint8_t *image, int16_t w, int16_t h) {
  ...
  uint32_t bufferIndex = 0;
	bufferIndex = x + 20480 - (y * _width);

  for(y=0; y<h; y=y+1){
    for(x=0; x<w; x=x+1){
			if (image[i] != 0x03) {	
				if (bufferIndex <= 20479) {
					buffer[bufferIndex] = image[i];
				}
			}
			bufferIndex ++;
      i = i + 1;                        // go to the next pixel
    }
		bufferIndex = bufferIndex - w - _width;
    i = i + skipC;
    i = i - 2*originalWidth;
  }
}
